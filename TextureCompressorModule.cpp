	...
	)
{
	...
	FVector4 SubAlphaCoverages(0, 0, 0, 0);
	...
	if(Settings.SubMipStartLevel > 0)
		const_cast<FTextureBuildSettings&>(Settings).SwitchSubMipSetting();

	// Filtering kernels.
	FImageKernel2D SubKernelSimpleAverage;
	FImageKernel2D SubKernelDownsample;
	SubKernelSimpleAverage.BuildSeparatableGaussWithSharpen(2);
	SubKernelDownsample.BuildSeparatableGaussWithSharpen(Settings.SharpenMipKernelSize, Settings.MipSharpening);

	//@TODO : add a true 3D kernel.

	EMipGenAddressMode SubAddressMode = ComputeAdressMode(Settings);
	bool bSubReDrawBorder = false;
	if (Settings.bPreserveBorder)
	{
		bSubReDrawBorder = !Settings.bBorderColorBlack;
	}
	...
	if (Settings.AlphaCoverageThresholds != FVector4(0, 0, 0, 0))
	{
		...
		{
		case MGTAM_Wrap:
			SubAlphaCoverages = ComputeAlphaCoverage<MGTAM_Wrap>(Settings.AlphaCoverageThresholds, AlphaScales, IntermediateSrcView);
			break;
		case MGTAM_Clamp:
			SubAlphaCoverages = ComputeAlphaCoverage<MGTAM_Clamp>(Settings.AlphaCoverageThresholds, AlphaScales, IntermediateSrcView);
			break;
		case MGTAM_BorderBlack:
			SubAlphaCoverages = ComputeAlphaCoverage<MGTAM_BorderBlack>(Settings.AlphaCoverageThresholds, AlphaScales, IntermediateSrcView);
			break;
		default:
			check(0);
		}
	...
	// Generate mips
	uint32 tempMipChainDepth = MipChainDepth;
	if (Settings.SubMipStartLevel > 0)
		const_cast<FTextureBuildSettings&>(Settings).SwitchSubMipSetting();
	...
	{
		const_cast<FTextureBuildSettings&>(Settings).SwitchSubMipSetting();
		if (Settings.SubMipStartLevel > 0 && tempMipChainDepth - MipChainDepth == Settings.SubMipStartLevel)
		{
			const_cast<FTextureBuildSettings&>(Settings).SwitchSubMipSetting();
			KernelSimpleAverage = SubKernelSimpleAverage;
			KernelDownsample = SubKernelDownsample;
			AddressMode = SubAddressMode;
			bReDrawBorder = bSubReDrawBorder;
			AlphaCoverages = SubAlphaCoverages;

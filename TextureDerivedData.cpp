	...
	)
{
	...
	TextureLODSettings.GetMipGenSettings( 
		Texture,
		(TextureMipGenSettings)Texture.MipGenSettings,
		MipGenSettings,
	...
	bool bSubDownsampleWithAverage;
	bool bSubSharpenWithoutColorShift;
	bool bSubBorderColorBlack;
	TextureMipGenSettings SubMipGenSettings;

	if (Texture.bUseSubMipGenSettings && Texture.MipGenSettings != Texture.SubMipGenSettings
		&& Texture.MipGenSettings != TMGS_NoMipmaps)
	{
		OutBuildSettings.SubMipStartLevel = Texture.SubMipStartLevel;
		bUseSubSetting = true;
		TextureLODSettings.GetMipGenSettings(
			Texture,
			(TextureMipGenSettings)Texture.SubMipGenSettings,
			SubMipGenSettings,
			OutBuildSettings.SubMipSharpening,
			OutBuildSettings.SubSharpenMipKernelSize,
			bSubDownsampleWithAverage,
			bSubSharpenWithoutColorShift,
			bSubBorderColorBlack
		);
	}
	...
	if (bUseSubSetting)
	{
		OutBuildSettings.SubMipGenSettings = SubMipGenSettings;
		OutBuildSettings.bSubDownsampleWithAverage = bSubDownsampleWithAverage;
		OutBuildSettings.bSubSharpenWithoutColorShift = bSubSharpenWithoutColorShift;
		OutBuildSettings.bSubBorderColorBlack = bSubBorderColorBlack;
	}
	else
	{
		OutBuildSettings.SubMipGenSettings = MipGenSettings;
		OutBuildSettings.bSubDownsampleWithAverage = bDownsampleWithAverage;
		OutBuildSettings.bSubSharpenWithoutColorShift = bSharpenWithoutColorShift;
		OutBuildSettings.bSubBorderColorBlack = bBorderColorBlack;
	}
	...
	if (bUseSubSetting)
	{
		OutBuildSettings.SubLODBias = TextureLODSettings.CalculateLODBias(SourceSize.X, SourceSize.Y, Texture.MaxTextureSize, Texture.LODGroup, Texture.LODBias, Texture.NumCinematicMipLevels, Texture.SubMipGenSettings, bVirtualTextureStreaming);
		OutBuildSettings.SubLODBiasWithCinematicMips = TextureLODSettings.CalculateLODBias(SourceSize.X, SourceSize.Y, Texture.MaxTextureSize, Texture.LODGroup, Texture.LODBias, 0, Texture.SubMipGenSettings, bVirtualTextureStreaming);
	}
	else
	{
		OutBuildSettings.SubLODBias = OutBuildSettings.LODBias;
		OutBuildSettings.SubLODBiasWithCinematicMips = OutBuildSettings.SubLODBiasWithCinematicMips;
	}
	...
	)
{
	...
	if (TPM)
	{
		...
		{
			/*if (Texture.bUseSubMipGenSettings && LayerIndex == Texture.SubMipStartLevel)
			{
				SourceBuildSettings.MipGenSettings = SourceBuildSettings.SubMipGenSettings;
				SourceBuildSettings.bDownsampleWithAverage = SourceBuildSettings.bSubDownsampleWithAverage;
				SourceBuildSettings.bSharpenWithoutColorShift = SourceBuildSettings.bSubSharpenWithoutColorShift;
				SourceBuildSettings.bBorderColorBlack = SourceBuildSettings.bSubBorderColorBlack;
				SourceBuildSettings.LODBias = SourceBuildSettings.SubLODBias;
				SourceBuildSettings.LODBiasWithCinematicMips = SourceBuildSettings.SubLODBiasWithCinematicMips;
...
static void GetBuildSettingsPerFormat(const UTexture& Texture, FTextureBuildSettings& SourceBuildSettings, const ITargetPlatform* TargetPlatform, TArray< TArray<FTextureBuildSettings> >& OutBuildSettingsPerFormat)
{
	const int32 NumLayers = Texture.Source.GetNumLayers();

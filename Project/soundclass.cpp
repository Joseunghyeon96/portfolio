...
bool SoundClass::PlayWaveFile()
{
	...
	result = m_secondaryBuffer1->SetVolume(m_vol);	
	if (FAILED(result))
	{
		return false;
	}
	...
	result = m_secondaryBuffer1->Play(0, 0, m_playMode);
	if (FAILED(result))
	{
		return false;
	}
...
void SoundClass::SetLoop(bool check)
{
	if (check == true)
		m_playMode = DSBPLAY_LOOPING;
	else
		m_playMode = 0;
}
...
void SoundClass::SetVol(int vol)
{
	m_vol = vol;
}

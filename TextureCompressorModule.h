...
{
	//Sub setting start level
	int8 SubMipStartLevel;
	/** Color adjustment parameters. */
	...
	float MipSharpening;
	/** This is sub value , The desired amount of mip sharpening. */
	float SubMipSharpening;
	...
	/** This is sub option, The size of the kernel with which mips should be sharpened. 2 for 2x2, 4 for 4x4, 6 for 6x6, 8 for 8x8 */
	uint32 SubSharpenMipKernelSize;
	...
	/** Sub Mipmap generation settings. */
	uint8 SubMipGenSettings; // TextureSubMipGenSettings, opaque to avoid dependencies on engine headers.
	...
	/** This is Sub Option. Whether each mip should use the downsampled-with-average result instead of the sharpened result. */
	uint32 bSubDownsampleWithAverage : 1;
	/** Whether sharpening should prevent color shifts. */
	uint32 bSharpenWithoutColorShift : 1;
	/**  This is Sub Option. Whether sharpening should prevent color shifts. */
	uint32 bSubSharpenWithoutColorShift : 1;
	/** Whether the border color should be black. */
	uint32 bBorderColorBlack : 1;
	/** This is Sub Option. Whether the border color should be black. */
	uint32 bSubBorderColorBlack : 1;
	...
	uint32 LODBias;
	/** This is sub obtion. The source texture's final LOD bias (i.e. includes LODGroup based biases) */
	uint32 SubLODBias;
	...
	/** This is sub option. The source texture's final LOD bias (i.e. includes LODGroup based biases). This allows cinematic mips as well. */
	uint32 SubLODBiasWithCinematicMips;
	...
	FTextureBuildSettings()
		...
		, bSubDownsampleWithAverage(false)
		, bSharpenWithoutColorShift(false)
		, bSubSharpenWithoutColorShift(false)
		, bBorderColorBlack(false)
		, bSubBorderColorBlack(false)
	...
	FORCEINLINE void SwitchSubMipSetting()
	{
		Swap(MipGenSettings, SubMipGenSettings);
		Swap(LODBias,SubLODBias);
		Swap(LODBiasWithCinematicMips,SubLODBiasWithCinematicMips);
		Swap(MipSharpening,SubMipSharpening);
		Swap(SharpenMipKernelSize,SubSharpenMipKernelSize);

		uint32 temp;
		temp = bDownsampleWithAverage;
		bDownsampleWithAverage = bSubDownsampleWithAverage;
		bSubDownsampleWithAverage = temp;

		temp = bSharpenWithoutColorShift;
		bSharpenWithoutColorShift = bSubSharpenWithoutColorShift;
		bSubSharpenWithoutColorShift = temp;

		temp = bBorderColorBlack;
		bBorderColorBlack = bSubBorderColorBlack;
		bSubBorderColorBlack = temp;
	}
};
